<?php
//本源码归许愿数据所有
function send_mail($email,$text,$sub){
     global $config;
	 if(!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)==false){
		   return 'Email格式错误！';
	  }
	  if(empty($config['smtp']) || empty($config['email']) || empty($config['epass'])){
	      return '配置错误';
	  }	 
     $mail = new Smtper();
     $mail->setServer($config['smtp'], $config['email'], $config['epass'], 465, 1); //设置smtp服务器
     $mail->setFrom($config['title'],$config['email']); //设置发件人
	 $mail->setReceiver($email);  //设置收件人
	 $mail->setMail($sub, $text); //设置邮件主题、内容
     $mail->send(); //发送
	 return ($mail->error());
}  
